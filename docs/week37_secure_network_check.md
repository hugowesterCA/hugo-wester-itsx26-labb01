...
Min dators publika ip adress ser ut såhär xx.xx.xxx.xxx
Ip adressen för servern (min VM på oracle cloud) xxx.xxx.xxx.xx.
...
| Lokal tjänst | curl http://localhost:8080 | Ja servern går att nå. när jag kör curl kommandot så hämtas en filista som finns i den mappen jag hostade servern på, i detta fall min egen mapp. |
...
| Felsöka syntax fel i skriptet | Jag fick förklarat att det var en if sats i if satsen och då måste man ha två fi i slutet för att avsluta båda if satserna så jag la till en rad med fi som föreslaget | Testade igen och då rullade skriptet som tänkt | Här la jag inte till något eget då det va ett så pass enkelt fel|
| Osäker på kod förslag från vscode | Jag frågade om netstat som förlsag från vscode men claude påminde om att ss va i uppgiften inte netstat, vscode gav även som förslag att ha > istället för >> men det hade skrivit över hela loggen istället för att lägga till (detta visste jag själv) | Jag valde ss och >> och testkörde för att se att port översikten funkade samt att loggen skrevs till och till skrev över någon annan logg. | Jag själv valde >> istället för > och jag valde ss efter att cloude påminde mig om att det va det som sas i uppgiften |
